...
3. Type search query to trigger hx-get="/admin/blog/tags/search" with _tag_search parameter
4. Verify suggestions appear in #tag-suggestions after 200ms delay
5. Click a suggestion (uses inline onclick handler)
...
10. Click X on tag chip to remove tag (uses this.parentElement.remove())
...
- **Remove tag**: Click X on "JavaScript" chip, chip and hidden input removed from DOM via this.parentElement.remove()
...
- Tag chip structure: <div class="tag-chip">TagName <button onclick="this.parentElement.remove()">X</button><input type="hidden" name="tag_ids" value=":id"></div>
- Quick-create button: hx-post="/admin/blog/tags/quick-create" hx-vals='{"name":"..."}' hx-target="#selected-tags" hx-swap="beforeend"
- JavaScript: Suggestion clicks use inline onclick handlers, removal uses this.parentElement.remove()

## HTMX Interactions
- **Tag search**: hx-get="/admin/blog/tags/search?_tag_search=keyword" hx-trigger="input changed delay:200ms, focus" hx-target="#tag-suggestions" (returns tag_suggestions.html partial with clickable suggestions)
...
- **Chip addition**: Clicking suggestion uses inline onclick handler which manually creates chip HTML and appends to #selected-tags
