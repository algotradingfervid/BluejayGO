...
7. Click "Delete All Specs" button with `hx-delete="/admin/products/:id/specs"` and `hx-confirm`
8. Confirm deletion dialog
9. Verify all specs removed from section
...
**Note**: Individual delete buttons appear in the template UI but the backend route (`/admin/products/:id/specs/:spec_id`) is NOT implemented. This is a known bug. Only bulk deletion via `DELETE /admin/products/:id/specs` is currently supported.
