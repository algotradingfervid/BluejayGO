...
import (
	...
	"fmt"           // Formatting the friendly upload error message
	"log/slog"      // Structured logging for error and debug output
	"net/http"      // HTTP status codes and request/response handling
	"path/filepath" // Extracting the uploaded file's extension for the error message
	"strings"       // Normalizing the extension to lowercase
	...
	"github.com/narendhupati/bluejay-cms/internal/services" // Shared services (file uploads, etc.)
)
...
func (h *HeaderHandler) Update(c echo.Context) error {
	// Resolve the logo path. The text field provides a fallback (paste/clear a path),
	// but if the admin uploaded a file it takes precedence: we persist it to disk and
	// use the returned public URL as the logo path.
	logoPath := c.FormValue("header_logo_path")
	if fileHeader, err := c.FormFile("header_logo_file"); err == nil && fileHeader != nil {
		path, uploadErr := h.uploadSvc.UploadLogo(fileHeader)
		if uploadErr != nil {
			h.logger.Error("failed to upload logo", "error", uploadErr)

			// Re-render the form with a friendly, in-form error instead of dumping
			// the admin out to a raw JSON error page. Name the offending extension
			// and list the supported formats and size limit.
			ext := strings.ToLower(filepath.Ext(fileHeader.Filename))
			friendly := fmt.Sprintf(
				"Couldn't upload logo: %q is not a supported image type. Use JPG, PNG, WEBP, or SVG (max 5MB).",
				ext,
			)

			settings, getErr := h.queries.GetSettings(c.Request().Context())
			if getErr != nil {
				h.logger.Error("failed to load settings", "error", getErr)
				return echo.NewHTTPError(http.StatusInternalServerError)
			}

			return c.Render(http.StatusBadRequest, "admin/pages/header_form.html", map[string]interface{}{
				"Title":    "Header Management",
				"Settings": settings,
				"Error":    friendly,
			})
